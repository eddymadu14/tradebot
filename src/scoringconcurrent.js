...
// ===== Detect Cross (1H current candle only) =====
...
function detectRecentCross(closes, lookbackCandles = 5) {
  ...
  const lastIndex = closes.length - 2; // ignore current forming daily candle

  for (let i = lastIndex; i > lastIndex - lookbackCandles; i--) {
    if (!maFast[i - 1] || !maFast[i] || !maSlow[i - 1] || !maSlow[i])
      continue;
  ...
  const tasks = pairs.map(symbol =>
    limit(async () => {
      try {
        ...
        if (cross1H !== dir4H) return;

        const recent1DCross = detectRecentCross(d1.closes, 5);
        if (!recent1DCross) return;
        if (recent1DCross !== cross1H) return;

        const dir1D = getDirection(d1.closes);
        if (cross1H !== dir1D) return;
        ...
        console.log(
          `\n🔥 ${symbol} — ${cross1H} ALIGNMENT + RECENT 1D CROSS | Score: ${score.total} | ${score.criteria.join(", ")}`
        );
      } catch {
        scannedCount++;
      }
