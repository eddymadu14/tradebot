...
async function run() {
  console.log("BTC Weekly Candle Anatomy\n");
  ...
  analyzed.forEach(c => {
    if (c.type === "BEAR") {
      console.log(
        `${c.week} | BEAR | Body: ${c.bodyPct}% | Upper Wick: ${c.upperWickPct}%`
      );
    }

    if (c.type === "BULL") {
      console.log(
        `${c.week} | BULL | Body: ${c.bodyPct}% | Lower Wick: ${c.lowerWickPct}%`
      );
    }
  });
}
