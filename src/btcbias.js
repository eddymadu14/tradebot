 ...
 * =========================
 */
async function getKlines(interval, startTime, endTime) {
...
function getCompletedDailyStructure(daily) {
  ...
  return { bullish, bearish, completed, previous };
}
...
function getSupportResistance(h4) {
  const completedCandles = h4.slice(0, -1);
  const lookback = completedCandles.slice(-20);
...
function calculateBias(data) {
  ...
  if (data.price > data.yesterdayHigh) {
    score += 20;
    conditions.push({ name: "Yesterday High/Low", status: "Bullish", score: 20 });
  } else if (data.price < data.yesterdayLow) {
    score -= 20;
    conditions.push({ name: "Yesterday High/Low", status: "Bearish", score: -20 });
  } else {
    conditions.push({ name: "Yesterday High/Low", status: "Neutral", score: 0 });
  }
  ...
  if (supportDistance <= 1.5) {
    score += 15;
    conditions.push({ name: "Support", status: "Near Support", score: 15 });
  } else if (resistanceDistance <= 1.5) {
    score -= 15;
    conditions.push({ name: "Resistance", status: "Near Resistance", score: -15 });
  } else {
    conditions.push({ name: "Support/Resistance", status: "Neutral", score: 0 });
  }
...
/**
 * =========================
 * MAIN ENGINE (HISTORICAL DAY)
 * =========================
 */
async function main() {
  console.log(`Running bias analysis for ${targetDate}...`);

  /**
   * 1. Get all data BEFORE and DURING that day
   */
  const daily = await getKlines("1d", dayStart, dayEnd);
  const h4 = await getKlines("4h", dayStart, dayEnd);

  if (daily.length < 3 || h4.length < 5) {
    console.log("Not enough data for this date range.");
    return;
  }

  /**
   * 2. Price = last candle of that day (completed state)
   */
  const livePrice = h4[h4.length - 1].close;

  /**
   * 3. Yesterday reference (relative to target day)
   */
  const yesterday = daily[daily.length - 1];

  /**
   * 4. Daily structure (completed candles only)
   */
  const dailyStructure = getCompletedDailyStructure(daily);

  /**
   * 5. 4H structure (inside target day)
   */
  ...
  const momentum = calculateMomentum(livePrice, dayOpen);
  ...
  console.log("\n==============================");
  console.log(`BTC BIAS REPORT: ${targetDate}`);
  console.log("==============================\n");
