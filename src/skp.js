...
// ===== Scoring Model (Realistic Thresholds) =====
function scoreSpike(candles) {
  ...
  if (rangeRatio >= 1.5) score += 2; // relaxed from 1.8
  else if (rangeRatio >= 1.3) score += 1;

  // 4️⃣ Commitment (0–1)
  if (bodyRatio >= 0.5) score += 1; // relaxed from 0.6

  // 5️⃣ Structure Break (0–1)
  if (last.close > recentHigh || last.close < recentLow) score += 1;

  // 6️⃣ Volatility Regime (0–1)
  if (atrRatio >= 1.2) score += 1; // relaxed from 1.5
...
// ===== Scanner =====
async function scan() {
  console.log("\n🚀 Realistic Futures Volume Spike Scanner\n");
  ...
  const tasks = pairs.map(symbol =>
    limit(async () => {
      try {
        ...
        if (result && result.score >= 5) { // relaxed from 6
