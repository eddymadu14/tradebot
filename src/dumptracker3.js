...
// DUMP AGE DETECTOR
// ----------------------------
function detectDumpAge(closes, opens) {
  let barsAgo = 0;

  for (let i = closes.length - 1; i >= 3; i--) {
    const c = closes[i];
    const o = opens[i];
  ...
  if (barsAgo >= 4 && barsAgo <= 8) {
    freshness = "ACTIVE";
  } else if (barsAgo >= 9 && barsAgo <= 15) {
    freshness = "AGING";
  } else if (barsAgo > 15) {
    freshness = "OLD";
  }
...
async function analyzeSymbol(meta, btcWeakness) {
  ...
  try {
    ...
    // Dump age detection
    // ----------------------------
    const dumpAge = detectDumpAge(c5, o5);
    ...
    return {
      ...
      dumpAge: `${dumpAge.minutes}m`,

      phase: dumpAge.freshness,

      bars: dumpAge.barsAgo,
