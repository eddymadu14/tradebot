...
async function analysePair(symbol) {
  ...
  try {
    ...
    const fundingData =
      await fetchFunding(symbol);

    const candles =
      await fetchCandles(symbol, "1h");
    ...
    const bearishProbability =
      Number(
        (
          (totalBearish / total) * 100
        ).toFixed(2)
      );

    // ======================================================
    // FILTER ONLY STRONG PAIRS
    // ======================================================

    if (
      bullishProbability >= MIN_PROBABILITY
    ) {
    ...
    if (
      bearishProbability >= MIN_PROBABILITY
    ) {
