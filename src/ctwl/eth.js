/**
 * CTWL-1H PRO — Directional Predictive Engine (ETH)
 * -------------------------------------------------
...
// MAIN
// ======================
export async function runCTWL1H_PREDICT_ETH(symbol = "ETHUSDT") {
  ...
  const volatility = volatilityGateETH(c1h);
...
function analyzeHTF(candles) {
  ...
  const EMA = (p) => {
    let k = 2 / (p + 1);
    const ema = [closes.slice(0, p).reduce((a, b) => a + b) / p];
    for (let i = p; i < closes.length; i++) {
      ema.push(closes[i] * k + ema.at(-1) * (1 - k));
    }
    return ema;
  };

  const ema20 = EMA(20).at(-1);
  const ema50 = EMA(50).at(-1);
  const ema100 = EMA(100).at(-1);
  const ema200 = EMA(200).at(-1);
  const lastClose = closes.at(-1);
  const slope20 = EMA(20).at(-1) - EMA(20).at(-2);

  const last5 = closes.slice(-6);
  const hhhl = last5.every((c, i, a) => i === 0 || c > a[i - 1]);
  const lllh = last5.every((c, i, a) => i === 0 || c < a[i - 1]);
  ...
  if (bull >= 2) return { trend: "BULL" };
  if (bear >= 2) return { trend: "BEAR" };
  return { trend: "RANGE" };
}
...
// ======================
// VOLATILITY (ETH TUNED)
// ======================
function volatilityGateETH(candles) {
  const fast = ATR(candles.slice(-14));
  const slow = ATR(candles.slice(-50));
  const expand = fast > slow * 1.08; // ETH adjustment
...
// ENTRY ZONE
// ======================
function buildEntryZone(candles, dir) {
  const c = candles.at(-1);
  if (dir === "BULL") return { entry: (c.low + c.close) / 2, invalidation: c.low };
  if (dir === "BEAR") return { entry: (c.high + c.close) / 2, invalidation: c.high };
  return null;
}
...
if (process.argv[1] === new URL(import.meta.url).pathname) {
  try {
    const res = await runCTWL1H_PREDICT_ETH("ETHUSDT");
    console.log("CTWL ETH RESULT ↓↓↓");
    console.log(JSON.stringify(res, null, 2));
  } catch (err) {
    console.error("CTWL ETH ERROR:", err.message);
  }
}
