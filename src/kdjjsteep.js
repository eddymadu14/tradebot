...
// ===== Detect KDJ Cross (UNCHANGED) =====
function detectKDJCross(highs, lows, closes) {
  const { K, D, J } = calculateKDJ(highs, lows, closes);
  const i = closes.length - 1;

  const K_now = K[i], K_prev = K[i - 1];
  const D_now = D[i], D_prev = D[i - 1];
  const J_now = J[i];

  if ([K_now, K_prev, D_now, D_prev, J_now].some(v => v === null)) return null;

  const bullishCross = K_prev < D_prev && K_now > D_now;
  const nearBullish = K_now <= 25 && K_now > D_now && (K_now - D_now <= 1);

  const bearishCross = K_prev > D_prev && K_now < D_now;
  const nearBearish = K_now >= 65 && K_now < D_now && (D_now - K_now <= 1);

  if (bullishCross || nearBullish) return "LONG";
  if (bearishCross || nearBearish) return "SHORT";
...
async function scan() {
  ...
  for (const { symbol, closes, highs, lows, volumes } of validSymbols) {
    ...
    const jSignal = detectJMomentum(highs, lows, closes);

    const score = checkFilters({ closes, highs, lows, volumes });

    // 🔥 ORIGINAL CROSS (UNCHANGED OUTPUT)
    if (cross) {
      console.log(`\n🔥 ${symbol} — ${cross} CROSS | Score: ${score.total} | ${score.criteria.join(", ")}`);
    }

    // ⚡ NEW J MOMENTUM (SEPARATE LOG)
    if (jSignal) {
      console.log(`\n⚡ ${symbol} — ${jSignal} J MOMENTUM | Score: ${score.total} | ${score.criteria.join(", ")}`);
    }
  }
