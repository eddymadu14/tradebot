...
// ---------------- FILTER: >2 repeating chars in private key (zeros ignored) ----------------
...
// ---------------- FILTER: >2 repeating chars in address ----------------
...
function loadProgress(startHex, endHex, targetAddress) {
    if (fs.existsSync(PROGRESS_FILE)) {
        try {
            ...
            if (
                (!data.version || data.version === PROGRESS_VERSION) &&
                ...
                console.log(`\n📂 Different session detected, starting fresh.\n`);
            }
        } catch {
...
async function scanRange(startHex, endHex, targetAddress) {
    ...
    while (true) {
        ...
        // Filter 1: skip private keys with >2 consecutive repeating non-zero chars
        if (privateKeyHasExcessiveRepeats(privKeyHex)) {
            skipped++;
            continue;
        }

        // Derive legacy address only if key passed filter 1
        const address = privateKeyToLegacyAddress(privKeyHex);

        // Filter 2: skip addresses with >2 consecutive repeating chars
