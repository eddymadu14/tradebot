...
function hasExcessiveRepeats(privKeyHex) {
    // Strip all zeros before checking
    const withoutZeros = privKeyHex.replace(/0/g, "");

    // If nothing left after stripping zeros, no repeats to check
...
async function scanRange(startHex, endHex, targetAddress) {
    ...
    while (true) {
        ...
        // ✅ Filter: skip private keys with >2 consecutive repeating chars (zeros ignored)
